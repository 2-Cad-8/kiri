...
let assets = ['/', '/index.html', '/css/estilos.css', '/css/avatars_icons.css' ,'/js/app.js', '/404.html'];
//starter images
let imageAssets = ['/img/1011-800x600.jpg', '/assets/img/girl.png'];
//TODO:
let DB = null;
              ...
              if (fetchResponse.status == 404) {
                ...
                if (
                  ...
                  return caches.open(imageName).then((cache) => {
                    return cache.match('/assets/img/girl.png');
                  });
                }
              }
            },
...
const savePerson = (person, clientId) => {
  if (person && DB) {
    let tx = DB.transaction('Preguntas', 'readwrite');
    tx.onerror = (err) => {
      //failed transaction
    };
    tx.oncomplete = (ev) => {
...
const openDB = (callback) => {
  let req = indexedDB.open('kiri', version);
  req.onerror = (err) => {
    //could not open db
    console.warn(err);
    DB = null;
